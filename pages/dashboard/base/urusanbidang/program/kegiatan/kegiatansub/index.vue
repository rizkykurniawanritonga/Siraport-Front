...
export default {
  async setup() {
    ...
    const [{ data: data, refresh: getData, pending }] = await Promise.all([
      useFetch(`/api/struktur/read?chain=base-kegiatanSub&id=${filter[1]}`),
    ]);
  ...
  methods: {
    ...
    async simpanUrusan() {
      ...
      $fetch("/api/struktur/add?chain=base-addKegiatanSub", {
        method: "post",
        body: {
          id_base_kegiatan: this.getF(1),
    ...
    async updateUrusan() {
      this.loadingbtn = true;
      $fetch(
        `/api/struktur/update?chain=base-updateKegiatanSub&id=${this.dataFocus}`,
        {
          method: "PUT",
          body: {
            id_base_kegiatan: this.getF(1),
    ...
    async hapusUrusan() {
      this.loadingbtn = true;
      $fetch(
        `/api/struktur/delete?chain=base-deleteKegiatanSub&id=${this.dataFocus}`,
        {
          method: "PUT",
        }
      ).then((delSt) => {
