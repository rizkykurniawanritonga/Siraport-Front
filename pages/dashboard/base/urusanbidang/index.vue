...
export default {
  async setup() {
    ...
    const [{ data: data, refresh: getData, pending }] = await Promise.all([
      useFetch(`/api/struktur/read?chain=base-urusanBidang&id=${filter[1]}`),
    ]);
  ...
  methods: {
    ...
    async simpanUrusan() {
      ...
      $fetch("/api/struktur/add?chain=base-addUrusanBidang", {
        method: "post",
        body: {
          id_base_urusan: this.getF(1),
    ...
    async updateUrusan() {
      this.loadingbtn = true;
      $fetch(
        `/api/struktur/update?chain=base-updateUrusanBidang&id=${this.dataFocus}`,
        {
          method: "PUT",
          body: {
            id_base_urusan: this.getF(1),
    ...
    async hapusUrusan() {
      this.loadingbtn = true;
      $fetch(
        `/api/struktur/delete?chain=base-deleteUrusanBidang&id=${this.dataFocus}`,
        {
          method: "PUT",
        }
      ).then((delSt) => {
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="flex gap-2 justify-center">
                      <button
                        @click="nextPage('program', dt.uraian + '|' + dt.id)"
          ...
          <Modal :show="showModal" @tutup="showModal = false">
            <template #content>
              <h3
                ...
                id="modal-title"
              >
                {{ modalMode == "add" ? "Tambah" : "Edit" }} Urusan Bidang
              </h3>
              <form>
                <div class="mb-4">
