...
export default {
  async setup() {
    ...
    const [{ data: data, refresh: getData, pending }] = await Promise.all([
      useFetch(`/api/struktur/read?chain=anggaran-program&id=${filter[1]}`),
    ]);
  ...
  methods: {
    ...
    async simpanUrusan() {
      ...
      $fetch("/api/struktur/add?chain=anggaran-addProgram", {
        method: "post",
        body: {
          id_urusan_bidang: this.getF(1),
    ...
    async updateUrusan() {
      this.loadingbtn = true;
      $fetch(
        `/api/struktur/update?chain=anggaran-updateProgram&id=${this.dataFocus}`,
        {
          method: "PUT",
          body: {
            id_urusan_bidang: this.getF(1),
    ...
    async hapusUrusan() {
      this.loadingbtn = true;
      $fetch(
        `/api/struktur/delete?chain=anggaran-deleteProgram&id=${this.dataFocus}`,
        {
          method: "PUT",
        }
      ).then((delSt) => {
      ...
      <div class="col-lg-12">
        <div class="card">
          <div class="card-header align-items-center d-flex">
            <h4 class="card-title mb-0 flex-grow-1">Program: {{ getF(0) }}</h4>
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="flex gap-2 justify-center">
                      <button
                        @click="nextPage('kegiatan', dt.uraian + '|' + dt.id)"
          ...
          <Modal :show="showModal" @tutup="showModal = false">
            <template #content>
              <h3
                ...
                id="modal-title"
              >
                {{ modalMode == "add" ? "Tambah" : "Edit" }} Program
              </h3>
              <form>
                <div class="mb-4">
