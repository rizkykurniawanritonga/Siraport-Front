...
export default {
  async setup() {
    ...
    const [{ data: data, refresh: getData, pending }] = await Promise.all([
      useFetch(
        `/api/struktur/read?chain=anggaran-kegiatanSubUraian&id=${filter[1]}`
      ),
    ]);
  ...
  methods: {
    ...
    async simpanUrusan() {
      ...
      $fetch("/api/struktur/add?chain=anggaran-addKegiatanSubUraian", {
        method: "post",
        body: {
    ...
    async updateUrusan() {
      this.loadingbtn = true;
      $fetch(
        `/api/struktur/update?chain=anggaran-updateKegiatanSubUraian&id=${this.dataFocus}`,
        {
          method: "PUT",
          body: {
            id_kegiatan_sub: this.getF(1),
    ...
    async hapusUrusan() {
      this.loadingbtn = true;
      $fetch(
        `/api/struktur/delete?chain=anggaran-deleteKegiatanSubUraian&id=${this.dataFocus}`,
        {
          method: "PUT",
        }
      ).then((delSt) => {
