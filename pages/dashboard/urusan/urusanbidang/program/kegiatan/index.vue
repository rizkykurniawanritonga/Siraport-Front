...
export default {
  async setup() {
    ...
    const [{ data: data, refresh: getData, pending }] = await Promise.all([
      useFetch(`/api/struktur/read?chain=anggaran-kegiatan&id=${filter[1]}`),
    ]);
  ...
  methods: {
    ...
    async simpanUrusan() {
      ...
      $fetch("/api/struktur/add?chain=anggaran-addKegiatan", {
        method: "post",
        body: {
          id_program: this.getF(1),
    ...
    async updateUrusan() {
      this.loadingbtn = true;
      $fetch(
        `/api/struktur/update?chain=anggaran-updateKegiatan&id=${this.dataFocus}`,
        {
          method: "PUT",
          body: {
            id_program: this.getF(1),
    ...
    async hapusUrusan() {
      this.loadingbtn = true;
      $fetch(
        `/api/struktur/delete?chain=anggaran-deleteKegiatan&id=${this.dataFocus}`,
        {
          method: "PUT",
        }
      ).then((delSt) => {
      ...
      <div class="col-lg-12">
        <div class="card">
          <div class="card-header align-items-center d-flex">
            <h4 class="card-title mb-0 flex-grow-1">Kegiatan: {{ getF(0) }}</h4>
          ...
          <Modal :show="showModal" @tutup="showModal = false">
            <template #content>
              <h3
                ...
                id="modal-title"
              >
                {{ modalMode == "add" ? "Tambah" : "Edit" }} Kegiatan
              </h3>
              <form>
                <div class="mb-4">
              ...
              ><div class="sm:flex sm:items-start">
                ...
                <div class="mt-2 sm:ml-4 sm:text-left">
                  <h3
                    ...
                    id="modal-title"
                  >
                    Hapus Kegiatan {{ titleModalDelte }}?
                  </h3>
                  <div class="mt-2">
