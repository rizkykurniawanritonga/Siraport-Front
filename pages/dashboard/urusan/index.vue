...
export default {
  ...
  methods: {
    ...
    async getData() {
      const Audt = await apiKoneksi("/anggaran/urusan");
    ...
    async editDialog(val) {
      const eftch = await apiKoneksi(`/anggaran/urusan/${val}`);
    ...
    async updateUrusan() {
      ...
      const efu = await apiKoneksi(
        `/anggaran/updateUrusan/${this.dataFocus}`,
        {
          body: {
            kode_rek: this.addkoderekening,
            uraian: this.adduraian,
    ...
    async hapusUrusan() {
      ...
      const efh = await apiKoneksi(
        `/anggaran/deleteUrusan/${this.dataFocus}`,
        {},
        "PUT"
      );
